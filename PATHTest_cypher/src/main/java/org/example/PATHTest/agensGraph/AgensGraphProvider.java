...
import org.example.PATHTest.agensGraph.gen.AgensGraphGraphGenerator;
...
public class AgensGraphProvider extends CypherProviderAdapter<AgensGraphGlobalState, AgensGraphSchema, AgensGraphOptions> {
    ...
    public CypherConnection createDatabaseWithOptions(MainOptions mainOptions, AgensGraphOptions specificOptions) throws Exception {
        ...
        try{
            ...
            con.executeStatement("DROP GRAPH IF EXISTS agens CASCADE");
            con.executeStatement("CREATE GRAPH agens");
            con.executeStatement("SET graph_path = agens");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return con;
