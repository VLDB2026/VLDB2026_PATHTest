    ...
    public void toTextRepresentation(StringBuilder sb) {
        ...
        if(orderBy != null && orderBy.size() != 0){
            if (DifferentialNonEmptyBranchOracle.needLabel()){
                // todo 针对nebula进行额外处理，其order clause 只支持部分语法
                List<IExpression> cloumList = orderBy.stream().filter(e->!(e instanceof GetPropertyExpression)).collect(Collectors.toList());
                Randomly r = new Randomly();
                if (cloumList.size() > 0){
                    sb.append(" ORDER BY ");
                    for(int i = 0; i < cloumList.size(); i++){
                        cloumList.get(i).toTextRepresentation(sb);
                        if(i != cloumList.size()-1){
                            sb.append(", ");
                        }
                    }
                    if (isOrderByDesc) {
